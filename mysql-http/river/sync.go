...
import (
	...
	apatodon "ezreal.com.cn/ez_river/mysql-http/client"
...
func (r *River) syncLoop() {
	...
	defer ticker.Stop()
	defer r.wg.Done()
	reqs := make([]*apatodon.BulkRequest, 0, 1024)
	...
	for {
		...
		if needFlush {
			// send data to apatodon
			// TODO: retry some times?
			if err := r.doBulk(reqs); err != nil {
				log.Errorf("do apatodon bulk err %v, close sync", err)
				r.cancel()
				return
			}
			reqs = reqs[0:0]
...
func (r *River) makeRequest(rule *Rule, action string, rows [][]interface{}) ([]*apatodon.BulkRequest, error) {
	reqs := make([]*apatodon.BulkRequest, 0, len(rows))
	...
	for _, values := range rows {
		...
		req := &apatodon.BulkRequest{Index: rule.Index, Type: rule.Type, ID: id, Parent: parentID}
		if action == canal.DeleteAction {
			req.Action = canal.DeleteAction
		} else {
...
func (r *River) makeInsertRequest(rule *Rule, rows [][]interface{}) ([]*apatodon.BulkRequest, error) {
	return r.makeRequest(rule, canal.InsertAction, rows)
}

func (r *River) makeDeleteRequest(rule *Rule, rows [][]interface{}) ([]*apatodon.BulkRequest, error) {
	return r.makeRequest(rule, canal.DeleteAction, rows)
}

func (r *River) makeUpdateRequest(rule *Rule, rows [][]interface{}) ([]*apatodon.BulkRequest, error) {
	if len(rows)%2 != 0 {
	...
	for i := 0; i < len(rows); i += 2 {
		...
		req := &apatodon.BulkRequest{Index: rule.Index, Type: rule.Type, ID: beforeID, Parent: beforeParentID}
		if beforeID != afterID || beforeParentID != afterParentID {
			req.Action = canal.DeleteAction
			req.ID = afterID
			req.Parent = afterParentID
...
func (r *River) makeInsertReqData(req *apatodon.BulkRequest, rule *Rule, values []interface{}) {
	req.Data = make(map[string]interface{}, len(values))
	req.Action = canal.InsertAction
...
//send data to apatodon
func (r *River) doBulk(reqs []*apatodon.BulkRequest) error {
	if len(reqs) == 0 {
		return nil
	}
	...
	for _, req := range reqs {
		if req.Data != nil {
			req.Data["sourceType"] = r.c.SourceType
			id, _ := strconv.ParseInt(req.ID, 10, 64)
			req.Data["sourceId"] = id
		}
		fmt.Printf("req%+v\n", req)
		switch req.Action {
		case ActionDelete:
			err := r.ap.Delete(req.Index, req.Type, req.ID)
			if err != nil {
				log.Errorf("sync docs err %v after binlog %s", err, r.canal.SyncedPosition())
				continue
			}
		case ActionUpdate:
			err := r.ap.Update(req.Index, req.Type, req.ID, req.Data)
			if err != nil {
				log.Errorf("sync docs err %v after binlog %s", err, r.canal.SyncedPosition())
				continue
			}
		case ActionCreate:
			err := r.ap.Create(req.Index, req.Type, req.Data)
			if err != nil {
				log.Errorf("sync docs err %v after binlog %s", err, r.canal.SyncedPosition())
				continue
			}
